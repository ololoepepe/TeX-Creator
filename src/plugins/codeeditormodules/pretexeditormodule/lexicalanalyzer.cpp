...
bool LexicalAnalyzer::checkTokenOrder(const QList<Token> &tokens, Token::Type current, QString *err)
{
    ...
    case Token::Unknown_Token:
        if (Token::BACKSLASH_Token != current)
            return bRet(err, tr("Expected backslash", "error"), false);
        break;
    ...
    case Token::REAL_Token:
        ...
        case Token::SPEC_FUNC_NAME_Token:
            return bRet(err, tr("Unexpected function name", "error"), false);
        case Token::LBRACE_Token:
            return bRet(err, tr("Unexpected opening brace", "error"), false);
        case Token::LBRACKET_Token:
            return bRet(err, tr("Unexpected opening bracket", "error"), false);
        default:
            break;
        }
        break;
    case Token::SHARP_Token:
        if (Token::BACKSLASH_Token != current && Token::INTEGER_Token != current)
            return bRet(err, tr("Expected integer or backslash", "error"), false);
        break;
    case Token::BACKSLASH_Token:
        if (Token::FUNC_NAME_Token != current && Token::SPEC_FUNC_NAME_Token != current)
            return bRet(err, tr("Expected function name", "error"), false);
        break;
    case Token::FUNC_NAME_Token:
        if (Token::LBRACE_Token != current && Token::RBRACE_Token != current)
            return bRet(err, tr("Expected opening or closing brace", "error"), false);
        break;
    case Token::SPEC_FUNC_NAME_Token:
        if (Token::LBRACE_Token != current)
            return bRet(err, tr("Expected opening brace", "error"), false);
        break;
    case Token::LBRACE_Token:
        switch (current) {
        case Token::SPEC_FUNC_NAME_Token:
        ...
        case Token::RBRACKET_Token:
            return bRet(err, tr("Unexpected closing bracket", "error"), false);
        default:
            break;
        }
        break;
    ...
    case Token::LBRACKET_Token:
        ...
        case Token::LBRACE_Token:
            return bRet(err, tr("Unexpected opening brace", "error"), false);
        case Token::RBRACE_Token:
            return bRet(err,tr("Unexpected closing brace", "error"), false);
        default:
            break;
        }
        break;
...
bool LexicalAnalyzer::matchBackslash(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith('\\');
    return matchedLength;
}

bool LexicalAnalyzer::matchFuncName2Args(const QString &s, int &matchedLength)
{
    return LexicalAnalyzer::matchFuncName(s, matchedLength);
}

bool LexicalAnalyzer::matchInput(const QString &s, QString &path, int &matchedLength)
{
    path.clear();
    matchedLength = 0;
    if (s.isEmpty())
...
bool LexicalAnalyzer::matchLeftBrace(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith('{');
    return matchedLength;
}

bool LexicalAnalyzer::matchLeftBracket(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith('[');
    return matchedLength;
}

bool LexicalAnalyzer::matchRightBrace(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith('}');
    return matchedLength;
}

bool LexicalAnalyzer::matchRightBracket(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith(']');
    return matchedLength;
}

bool LexicalAnalyzer::matchSharp(const QString &s, int &matchedLength)
{
    matchedLength = s.startsWith('#');
    return matchedLength;
}

Token LexicalAnalyzer::parceFuncName(const QString &s, int pos)
{
    if (s.isEmpty())
        return Token();
...
Token LexicalAnalyzer::parceInteger(const QString &s, int pos)
{
    bool ok = false;
    int x = s.toInt(&ok);
    if (!ok)
...
Token LexicalAnalyzer::parceReal(const QString &s, int pos)
{
    bool ok = false;
...
Token LexicalAnalyzer::parceSpecFuncName(const QString &s, int pos)
{
    if (s.isEmpty())
        return Token();
...
Token LexicalAnalyzer::parceSpecialSymbol(const QString &s, int pos)
{
...
Token LexicalAnalyzer::parceString(const QString &s, int pos)
{
...
QString LexicalAnalyzer::replaceEscaped(QString s)
{
    int i = 0;
    bool escMod = false;
    while (i < s.length()) {
